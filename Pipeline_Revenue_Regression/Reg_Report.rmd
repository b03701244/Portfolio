...
```{r plot.diff, include=F, echo=T, message=F, warning=F}
library(cowplot)
library(ggplot2)
...
```{r show.plot, echo=T, include=T, message=F, warning=F}
plot_grid(plot.1, plot.2, plot.3, ncol = 3)
```

```{r anova, echo=T, include=F, message=F, warning=F}
library(car)
library(gridExtra)
options(scipen = 1)
...
```{r anova.show, echo=T, include=T, message=F, warning=F}
...
The linear model should look something like this:
...
## Results (A):
```{r plot.fit, echo=T, include = T, message=F, warning=F}
summary(dat.fit)
```
...
```{r plot.dir, echo=T, include=T, message=F, warning=F}
...
```{r plot.seg, echo=T, include=T, message=F, warning=F}
...
```{r plot.reg, echo=T, include=T, message=F, warning=F}
...
```{r goal.fit, echo=T, include=T, message=F, warning=F}
...
```{r plot.goal, echo=T, include=T, message=F, warning=F}
plot.goal <- dat %>%
...
```{r goal.pred, echo=T, include=F, message=F, warning=F}
options(scipen=999)
PAV.pred <- c()
...
```
```{r goal.table, echo=T, include=T, message=F, warning=F}
options(knitr.kable.NA = "")
pred.tbl <- pred.tbl %>%
